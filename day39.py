def earliest_finish_time(land_start, land_duration, water_start, water_duration):
    ...
    for i in range(len(land_start)):
        ...
        for j in range(len(water_start)):
            water_begin = max(land_finish, water_start[j])
            finish_time = water_begin + water_duration[j]
    ...
    for j in range(len(water_start)):
        ...
        for i in range(len(land_start)):
            land_begin = max(water_finish, land_start[i])
            finish_time = land_begin + land_duration[i]
